...
async fn invoice() {
    ...
    stop_node(node1_addr).await;
}

#[serial_test::serial]
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
#[traced_test]
async fn zero_amount_invoice_payment() {
    initialize();
    ...
    open_channel(node1_addr, &node2_pubkey, Some(NODE2_PEER_PORT), None, None, None, None).await;
    ...
    wait_for_balance(node1_addr, &None).await;

    // Verify invoice status is succeeded
    let status = invoice_status(node2_addr, &invoice).await;
    assert!(
        matches!(status, InvoiceStatus::Succeeded),
        "Invoice status should be Succeeded, got: {:?}",
        status
    );

    stop_node(node1_addr).await;
    stop_node(node2_addr).await;
}
